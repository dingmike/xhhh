...
<script>
  ...
  import { fundProductList, fundingProductDetail, deleteFund, overallBonus, singleBonus, adminRecord, memberRecord } from '@/api/fund'
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        memberRecord(this.listQuery).then(response => {
...
//        this.$router.push({path: '/news/addNews', query: {id: row.id}})
      ...
      goBack(){
        this.$router.push({path: '/fund-product/fund-scenery/fund-user-List'})
      }
    }
  }
</script>
